            ...
            for (int i = 0; i < 5; i++)
            {
                getGameRes.Add(new()
                {
                    ...
                    GameDetails = new(
                        ...
                        platform: [.. App.Fake.PickRandom(DomainGameDetails.ValidPlatforms, App.Fake.Random.Int(1, DomainGameDetails.ValidPlatforms.Count))],
                        tags: string.Join(", ", App.Fake.Lorem.Words(5)),
                        gameMode: App.Fake.PickRandom(DomainGameDetails.ValidGameModes.ToArray()),
                        distributionFormat: App.Fake.PickRandom(DomainGameDetails.ValidDistributionFormats.ToArray()),
