...
namespace TC.CloudGames.Api.Tests.Endpoints.User
{
    public class GetUserByEmailEndpointTests(App App) : TestBase<App>
    {
        ...
        {
            // Arrange
            var httpContext = App.Services.GetRequiredKeyedService<IHttpContextAccessor>(nameof(App.ValidUserContextAccessor));
            var userContext = App.Services.GetRequiredKeyedService<IUserContext>(nameof(App.ValidLoggedUser));
        ...
        /*
         * Will keep this code as backup, will be removed on next commits
         */
        //[Fact]
